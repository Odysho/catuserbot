...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    november.remove_event_handler(i)
    ...
    try:
        name = f"userbot.plugins.{shortname}"
        for i in reversed(range(len(november._event_builders))):
            ev, cb = november._event_builders[i]
            if cb.__module__ == name:
                del november._event_builders[i]
    except BaseException:
        raise ValueError
